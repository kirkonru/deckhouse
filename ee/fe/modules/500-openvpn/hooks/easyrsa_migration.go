...
}, dependency.WithExternalDependencies(migration))
...
func migration(input *go_hook.HookInput, dc dependency.Container) error {
	migrated := false
	...
	if len(statefulsets) > 0 {
		if migrated && statefulsets[0].(string) != "true" {
			...
			err = kubeClient.AppsV1().StatefulSets("d8-openvpn").Delete(context.TODO(), "openvpn", metav1.DeleteOptions{})
